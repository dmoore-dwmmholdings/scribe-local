...
//!
//! Run against the dev Postgres (design header):
//! ```text
...
async fn full_api_flow() {
    ...
    let (status, body) = call(
        ...
        Request::builder()
            ...
            .header("content-type", "application/json")
            .body(Body::from(json!({ "duration_ms": 60_000 }).to_string()))
            .unwrap(),
    )
    .await;
    ...
    // Recording flipped to processing.
    ...
    // Detail now carries the summary + transcript.
    let (status, body) = call(
        &app,
        Request::builder()
            .uri(format!("/recordings/{rec_id}"))
    ...
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["summary"]["title"], "Q3 planning");
